...
_EXT_MAP: dict[str, tuple[str, str]] = {
    ...
    ".csh":    ("shell",  "shell"),
    # ── BI tools ─────────────────────────────────────────────────────────
    ".pbit":   ("powerbi_binary", "json"),  # ZIP archive — must unzip first
...
def _extract_powerbi_binary(content_bytes: bytes, label: str) -> list[ExtractedContent]:
    """Extract content from .pbit/.pbix (ZIP-based Power BI files).

    Returns two kinds of chunks:
    1. dialect='powerbi'  — the raw DataModel JSON, for column-level lineage
       extraction in column_lineage_extractor (reads dataModel.tables directly)
    2. dialect='sql'      — any embedded SQL queries found inside the JSON
    """
    results: list[ExtractedContent] = []
    try:
        with zipfile.ZipFile(io.BytesIO(content_bytes)) as zf:
            for name in zf.namelist():
                if "DataModel" in name or name.endswith(".json"):
                    try:
                        ...
                        # Chunk 1: full DataModel JSON with dialect='powerbi'
                        # so column_lineage_extractor can read dataModel.tables
                        results.append(ExtractedContent(
                            content=raw,
                            dialect="powerbi",
                            label=f"{label}/{name}",
                        ))
                        # Chunk 2: any embedded SQL extracted from the JSON
                        results.extend(_extract_json(raw, f"{label}/{name}"))
                    except Exception:
                        pass
    except zipfile.BadZipFile:
        # Not a ZIP — try plain JSON (test/sample .pbit files)
        try:
            ...
            results.extend(_extract_json(raw, label))
        except Exception:
            pass
    return results
